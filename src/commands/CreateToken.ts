...
export default class CreateToken extends Command {
  ...
  handleOutput(data: any) {
    ...
    if (/^\S{64}$/.test(message.trim())) {
      ...
      const currentTokenKey = token.slice(0, 8);
      tokens.push(currentTokenKey);
      ...
      this.webview.postMessage({
        ...
        payload: { tokens: tokens, currentToken: currentTokenKey }, // Attention: currentToken and currentTokenKey are NOT the same thing, but here the naming is WRONG since from first version, so let's keep it... i.e. originally it should be currentTokenKey instead of currentToken
      });
    }
  }
}
